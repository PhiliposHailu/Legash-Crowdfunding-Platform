...
const container = {
  hidden: { opacity: 0 },
  seen: { opacity: 1, transition: { staggerChildren: 0.2, delayChildren: 0.5 } },
};
  ...
  return (
    <section className="relative w-full bg-primary-light overflow-hidden">
      <Container className="relative min-h-[550px] flex items-center justify-center py-12">
        <motion.ul variants={container} initial="hidden" animate="seen">
        ...
        <motion.div variants={heroTextConainer} initial="hidden" animate="seen"
          className={cn(
        ...
        >
          <motion.div variants={heroTextItem} className="px-3 py-1 bg-secondary-light rounded-full border border-secondary/20">
            <p className="text-secondary font-bold text-[10px] uppercase tracking-[0.2em]">
              Verified Fundraising
            </p>
          </motion.div>

          <motion.div variants={heroTextItem} className="space-y-3">
